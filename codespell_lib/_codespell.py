...
# default INI and TOML configuration files
_default_cfg_files = ("setup.cfg", ".codespellrc")
_default_toml_files = ("pyproject.toml",)
...
def create_parser() -> argparse.ArgumentParser:
    """Put all possible options into argparse.ArgumentParser."""
...
def parse_options(
    ...
    # Prepare INI config files.
    cfg_files = OrderedDict([(cfg_file, False)
                             for cfg_file in _default_cfg_files
                             if os.path.isfile(cfg_file)])
    if options.config:
        cfg_files[options.config] = True

    # Prepare TOML config files.
    toml_files = OrderedDict([(toml_file, False)
                              for toml_file in _default_toml_files
                              if os.path.isfile(toml_file)])
    if options.toml:
        toml_files[options.toml] = True

    # Initialise configuration.
    config = configparser.ConfigParser(interpolation=None)

    # Read TOML config files first.
    if toml_files:
        ...
        if tomllib is not None:
            for toml_file, explicit in toml_files.items():
                try:
                    with open(toml_file, "rb") as f:
                        try:
                            ...
                            if explicit:
                                print(
                                    f"ERROR: invalid TOML file '{f.name}': {e}",
                                    file=sys.stderr,
                                )
                                raise tomllib.TOMLDecodeError(
                                    f"invalid TOML file '{f.name}': {e}"
                                ) from None
                            else:
                                print(
                                    f"WARNING: invalid TOML file '{f.name}': {e}",
                                    file=sys.stderr,
                                )
                                continue
                    ...
                    if explicit:
                        ...
                        print(
                            f"WARNING: cannot access '{e.filename}': " f"{e.strerror}",
                            file=sys.stderr,
                        )
                        continue
                config.read_dict(data)

    # Find which INI config files are going to be used.
    used_cfg_files = []
    for cfg_file, explicit in cfg_files.items():
        _cfg = configparser.ConfigParser(interpolation=None)
        try:
            _cfg.read(cfg_file)
        except configparser.Error as e:
            print(
                f"WARNING: invalid configuration file '{cfg_file}': {e}",
                file=sys.stderr,
            )
            continue
        else:
    ...
    # Read the INI config files that are going to be used.
...
def parse_file(
    ...
    if filename == "-":
        ...
        try:
            ...
            print(f"WARNING: cannot access {filename}: {e.strerror}", file=sys.stderr)
            return bad_count
        except OSError:
            return bad_count
