...
class Ball:
    ...
    def __init__(self, canvas, x1, y1, x2, y2):
        ...
        self.ball = canvas.create_oval(
            self.x1, self.y1, self.x2, self.y2, fill="red", tags="dot1"
        )
...
class bar:
    ...
    def __init__(self, canvas, x1, y1, x2, y2):
        ...
        self.rod = canvas.create_rectangle(
            self.x1, self.y1, self.x2, self.y2, fill="yellow", tags="dot2"
        )
...
def score_board():
    ...
    button3 = Button(
        canvas2, text="PLAY AGAIN", bg="green", command=lambda: play_again(root2)
    )
    button3.pack()

    button4 = Button(
        canvas2, text="EXIT", bg="green", command=lambda: exit_handler(root2)
    )
    button4.pack()
